...
class RecipeTree(NodeMixin):
    ...
    def __init__(self, categories: List[AgentCategory], category_indices: List[Any]):
        ...
        if len(category_indices) % 2 != 0:
            raise ValueError(
                "RecipeTree must be initialized with an even-length list, containing indices and their children.")
    ...
    def combined_values_detailed_with_counters(self) -> list:
        ...
        if len(self.children) == 0:
            ...
            children_combined_values_detailed_list = [child.combined_values_detailed_with_counters() for child in
                                                      self.children]
            # put all together:
            children_values = []
            for child in self.children:
    ...
    def combined_values_detailed(self) -> list:
        ...
        if len(self.children) == 0:
            ...
            children_values.sort(reverse=True,
                                 key=lambda x: sum(x) if (isinstance(x, list) or isinstance(x, tuple)) else x)
    ...
    def optimal_trade_with_counters(self) -> (list, int, float, int, int):
        ...
        * third is the optimal GFT.
        ...
        for value in optimal_trade_values:
            value[1]['counter'] += 1
        kmin = optimal_trade_values[0][1]['counter'] if len(optimal_trade_values) > 0 else MAX_VALUE
        kmax = 0
    ...
    def largest_categories(self, indices=False) -> (int, list):
        ...
        if self_category_size > children_category_size or children_category_size == 0:
            ...
            for child in self.children:
                (child_largest_category_size, child_combined_category_size,
                 child_largest_categories) = child.largest_categories(indices=indices)
    ...
    def num_of_deals_explained(self, prices: List[float], add_num_of_overall_deals=True) -> tuple[
        ...
        if len(self.children) == 0:
            ...
            children_num_of_deals_explained = [child.num_of_deals_explained(prices, add_num_of_overall_deals=False) for
            ...
            kmin = kmax
            for child_kmin in children_kmins:
                if child_kmin > 0 and child_kmin < kmin:
                    kmin = child_kmin
            ...
            if num_of_deals == self_num_of_deals:
                explanation += "{}: all {} traders selected, price={}\n".format(self.name, self_num_of_deals,
                                                                                self_price)
            else:
                explanation += "{}: {} out of {} traders selected, price={}\n".format(self.name, num_of_deals,
                                                                                      self_num_of_deals, self_price)
            if num_of_deals == sum_children_num_of_deals:
                ...
                explanation += "{}: {} out of {} traders selected\n".format(sum_children_names, num_of_deals,
                                                                            sum_children_num_of_deals)
