...
def experiment(results_csv_file: str, auction_function: Callable, auction_name: str, recipe: tuple, value_ranges: list,
               nums_of_agents: list, num_of_iterations: int):
    """
    Run an experiment similar to McAfee (1992) experiment on the given auction.

    :param results_csv_file: The file name to write the results in it.
    ...
    for num_of_agents_per_category in nums_of_agents:
        # count the number of deals done in the optimal vs. the actual auction.
        sum_optimal_count = sum_auction_count = 0
        ...
        results_table.add(OrderedDict((
            ...
            ("market_gft_ratio",
             0 if sum_optimal_gft == 0 else round(sum_auction_market_gft / sum_optimal_gft * 100, 2)),
        )))
    results_table.done()
