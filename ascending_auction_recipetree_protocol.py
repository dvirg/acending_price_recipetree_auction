...
class TradeWithMultipleRecipes(Trade):
    ...
    def __init__(self, categories: List[AgentCategory], recipe_tree: RecipeTree, prices: List[float]):
        ...
        self.prices = prices
        (self.num_of_deals_cache, self.num_of_deals_explanation_cache, self.kmin,
         self.kmax) = recipe_tree.num_of_deals_explained(prices)
...
def budget_balanced_ascending_auction(
    ...
    while True:
        ...
        logger.info("Largest category indices are %s. Largest category size = %d, combined category size = %d",
                    indices_of_prices_to_increase, largest_category_size, combined_category_size)

        if combined_category_size == 0:
            logger.info("\nCombined category size is 0 - no trade!")
            logger.info("  Final price-per-unit vector: %s", prices)
            logger.info(remaining_market)
            return TradeWithMultipleRecipes(remaining_market.categories, recipe_tree,
