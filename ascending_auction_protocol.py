...
def budget_balanced_ascending_auction(market: Market, ps_recipe: list, max_iterations=9999999) -> TradeWithSinglePrice:
    """
    ...
    fractional_potential_ps = lambda category_index: remaining_market.categories[category_index].size() / ps_recipe[
        category_index]
    integral_potential_ps = lambda category_index: math.floor(
        remaining_market.categories[category_index].size() / ps_recipe[category_index])
    ...
    while True:
        ...
        logger.info("Chosen category: {} with {} agents and ratio {}".format(main_category.name, main_category.size(),
                                                                             fractional_potential_ps(
                                                                                 main_category_index)))
        ...
        logger.info(
            "  {} price increases to {}: {} agents and ratio {}".format(main_category.name, prices[main_category_index],
                                                                        main_category.size(),
